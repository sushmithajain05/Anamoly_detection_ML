...
def detect_anomalies_fft_dynamic():
    freq = np.fft.fft(df["power_load"])
    magnitude = np.abs(freq)

    magnitude_series = pd.Series(magnitude, index=df.index)

    roll_mean = magnitude_series.rolling(window=20, min_periods=1).mean()
    roll_std = magnitude_series.rolling(window=20, min_periods=1).std()

    dynamic_threshold = roll_mean + (2 * roll_std)

    df_anomalies = df[magnitude_series > dynamic_threshold]
...
def save_anomalies_to_csv():
    anomalies_df = detect_anomalies_fft_dynamic()
    anomalies_df.to_csv("anomalies_detected_dynamic.csv", index=False)
    print("Anomalies saved to anomalies_detected_dynamic.csv")

save_anomalies_to_csv()

app = Dash(__name__)
app.layout = html.Div([
    html.H1("Anomaly Detection using FFT with Dynamic Threshold"),
    dcc.DatePickerRange(
        id='date-picker',
...
def update_graph(start_date, end_date):
    ...
    df_anomalies = detect_anomalies_fft_dynamic()
    ...
    fig.update_layout(title="Anomaly Detection using FFT with Dynamic Threshold",
