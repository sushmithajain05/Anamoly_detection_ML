...
def detect_anomalies_esd(max_outliers=50, alpha=0.1):
    ...
    for i in range(max_outliers):
        ...
        critical_value = ((n - i - 1) * t.ppf(1 - alpha / (2 * (n - i)), n - i - 2)) / np.sqrt((n - i - 2) + t.ppf(1 - alpha / (2 * (n - i)), n - i - 2) ** 2)
        
        if test_statistic > critical_value:
            anomalies.append(max_index)
            series.drop(max_index, inplace=True)
        else:
            break
    
    df_anomalies = df.loc[anomalies].copy()
...
def save_anomalies_to_csv():
    anomalies_df = detect_anomalies_esd()
    anomalies_df.to_csv("anomalies_detected_esd.csv", index=False)
...
def update_graph(start_date, end_date):
    ...
    df_anomalies = detect_anomalies_esd()
